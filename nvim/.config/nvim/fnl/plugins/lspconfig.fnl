...
[{1 :L3MON4D3/LuaSnip
 ...
 {1 :hrsh7th/nvim-cmp
  ...
  :config (fn []
            (let [cmp (require :cmp)
                  cmp-sources (cmp.config.sources [{:name :nvim_lua}
                                                   {:name :nvim_lsp}
                                                   {:name :luasnip}
                                                   {:name :conjure}
                                                   {:name :buffer}
  ...
  :config (fn []
            (let [mason (require :mason)
              ...
              (mason-lsp.setup {:ensure_installed [:lua_ls
                                                   ...
                                                   :nil_ls
                                                   :terraformls]})
              (mason-null-ls.setup {:ensure_installed [:gofumpt
                                                       :delve
                                                       :goimports
                                                       :luacheck
                                                       :shellcheck
            ...
            (let [lsp (require :lspconfig)
                  ...
                  on-attach (fn [client bufnr]
                              (do
                                ...
                                (nset bufnr :n :<leader>rn
                                      ":lua vim.lsp.buf.rename()<CR>"
                                      (opts-with-desc "rename ident"))
                                (nset bufnr :n :<leader>ca
                                      ":lua vim.lsp.buf.code_action()<CR>"
                                      (opts-with-desc "code action"))
                                (nset bufnr :n :gd
                                      ":lua vim.lsp.buf.definition()<CR>"
                                      (opts-with-desc "go to definition"))
                                (nset bufnr :n :gD
                                      ":lua vim.lsp.buf.declaration()<CR>"
                                      (opts-with-desc "go to declaration"))
                                (nset bufnr :n :gr
                                      ":lua vim.lsp.buf.references()<CR>"
                                      (opts-with-desc "show references"))
                                (nset bufnr :n :<leader>ds
                                      ":lua require('telescope.builtin').lsp_document_symbols()<cr>"
                                      (opts-with-desc "show symbols"))
                                (nset bufnr :n :<leader>dr
                                      ":lua require('telescope.builtin').lsp_references()<cr>"
                                      (opts-with-desc "show lsp references"))
                                (nset bufnr :n :K
                                      "<Cmd>lua vim.lsp.buf.hover()<CR>"
                                      (opts-with-desc "lsp hover"))
                                (nset bufnr :n :<a-k>
                                      "<Cmd>lua vim.lsp.buf.signature_help()<CR>"
                                      (opts-with-desc "show signature help"))
                                (nset bufnr :n :<space>a
                                      "<Cmd>lua vim.lsp.buf.format()<CR>"
                                      (opts-with-desc "run code formatter"))))
                  quick-setups [:lua_ls
                                :terraformls
                                :tflint
                                :sqlls
                                ...
                                :clangd
                                :templ
                                :zls]] ; these setups don't require config outside capabilities and on_attach
              (tset capabilities.textDocument.completion.completionItem
                    :snippetSupport true)
              (lsp.fennel_language_server.setup {:on_attach on-attach
                                                 : capabilities
                                                 :settings {:fennel {:workspace {:library (vim.api.nvim_list_runtime_paths)
                                                                                 :checkThirdParty false}
                                                                     :diagnostics {:globals [:vim
                                                                                             :Snacks]}}}})
              (lsp.gopls.setup {:cmd [:gopls]
                                :on_attach on-attach
                                : capabilities
              ...
              (each [_ val (ipairs quick-setups)]
                ((. lsp val :setup) {: capabilities :on_attach on-attach}))))}]
