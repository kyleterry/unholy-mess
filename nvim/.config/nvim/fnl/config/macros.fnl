...
(fn map [modes lhs rhs ?opt]
  (assert-compile (or (= (type modes) :string) (sequence? modes))
                  "mode must be string or list" modes)
...
(local opts {})
(fn opts.default [] {:noremap true :unique true})

(local silent-opts {:noremap true :silent true :unique true})

(local expr-opts {:noremap true
                  :unique true
                  :expr true
                  :replace_keycodes false})

{: autocmd : autocmds : augroup : map}
